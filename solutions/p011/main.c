...
int main(void) {
  const char window_size = 4;
  const char row_len = 20;
  ...
  // Largest possible value is 9^4 which is 96,059,601
  int largest = 0;

  // Move input into array
  for (int i = 0; i < strlen(input); i+=3)
    array[i/3] = (input[i]-48)*10 + input[i+1]-48;
  

  //there is undefined behavior (different output when running multiple times)
